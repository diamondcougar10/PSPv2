    ...
    if (const auto* keyPressed = event.getIf<sf::Event::KeyPressed>()) {
        if (keyPressed->code == sf::Keyboard::Key::Left) {
            if (!themes_.empty()) {
                ...
                // Reset animations for new selection
                selectedScale_ = 1.0f;
                previewAlpha_ = 0.f;
                ...
                cameraOffsetX_ = targetCameraOffsetX_;  // Snap immediately
                updateLayout();
            }
...
void ThemeSelector::draw(sf::RenderWindow& window) {
    ...
    // Debug output
    std::cout << "Selected: " << selectedIndex_ << " Camera: " << cameraOffsetX_ << " Target: " << targetCameraOffsetX_ << std::endl;
    ...
    for (int clone = -clonesNeeded; clone <= clonesNeeded; ++clone) {
        for (size_t i = 0; i < themes_.size(); ++i) {
            ...
            // Only the main instance (clone 0) can be selected
            bool isSelected = (i == selectedIndex_) && (clone == 0);
            auto& theme = themes_[i];
